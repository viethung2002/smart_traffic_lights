...
int main(int argc, char *argv[])
{
  ...
  // Khởi tạo socket trước khi chạy pipeline
  initialize_socket();
  ...
  for (i = 0; i < num_instances; i++)
  {
    ...
    for (j = 0; j < appCtx[i]->config.num_sink_sub_bins; j++)
    {
      ...
      windows[i] = XCreateSimpleWindow(display, RootWindow(display, DefaultScreen(display)), hints.x, hints.y, width, height, 2,
  ...
  // if (sock > 0)
  // {
  //   close(sock); // Đóng socket khi thoát
  //   sock = 0;
  // }
